...
NO_TARGET_ERR = {"status": 400,
                 "error":
                 "Please provide a url in the ?target query parameter."}
NO_HTTPS = {"status": 400,
            "error":
            "Please provide a url to an HTTPS source."}
NO_MIME_TYPE_ERRO = {"status": 400,
                     "error":
                     "Please provide a url locating an image."}
inferer = LearnerInferer(Config().MODEL_PATH, Config().MODEL_FILE)
# TODO: Move route logic out of controller.
